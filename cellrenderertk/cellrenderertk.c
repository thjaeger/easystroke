...
static void gtk_cell_renderer_tk_get_property (GObject         *object,
                                                  guint            param_id,
                                                  GValue          *value,
                                                  GParamSpec      *pspec);
static void gtk_cell_renderer_tk_set_property (GObject         *object,
                                                  guint            param_id,
                                                  const GValue    *value,
                                                  GParamSpec      *pspec);
static void gtk_cell_renderer_tk_get_size     (GtkCellRenderer *cell,
                                                  GtkWidget       *widget,
...
static GtkCellEditable *
           gtk_cell_renderer_tk_start_editing (GtkCellRenderer *cell,
                                                  GdkEvent        *event,
                                                  GtkWidget       *widget,
                                                  const gchar     *path,
...
G_DEFINE_TYPE (GtkCellRendererTK, gtk_cell_renderer_tk, GTK_TYPE_CELL_RENDERER_TEXT)

static void
gtk_cell_renderer_tk_init (GtkCellRendererTK *cell_accel)
{
}

static void
gtk_cell_renderer_tk_class_init (GtkCellRendererTKClass *cell_accel_class)
{
  ...
  object_class->set_property = gtk_cell_renderer_tk_set_property;
  object_class->get_property = gtk_cell_renderer_tk_get_property;

  cell_accel_class->base_get_size = cell_renderer_class->get_size;
  cell_accel_class->base_start_editing = cell_renderer_class->start_editing;

  cell_renderer_class->get_size      = gtk_cell_renderer_tk_get_size;
  cell_renderer_class->start_editing = gtk_cell_renderer_tk_start_editing;

  /**
   * GtkCellRendererTK:accel-key:
   *
  ...
  /**
   * GtkCellRendererTK::accel-edited:
  ...
  signals[ACCEL_EDITED] = g_signal_new (I_("accel-edited"),
					GTK_TYPE_CELL_RENDERER_TK,
					G_SIGNAL_RUN_LAST,
					G_STRUCT_OFFSET (GtkCellRendererTKClass, accel_edited),
					NULL, NULL,
  ...
  /**
   * GtkCellRendererTK::accel-cleared:
   * @accel: the object reveiving the signal
   * @path_string: the path identifying the row of the edited cell
   *
  ...
  signals[ACCEL_CLEARED] = g_signal_new (I_("accel-cleared"),
					 GTK_TYPE_CELL_RENDERER_TK,
					 G_SIGNAL_RUN_LAST,
					 G_STRUCT_OFFSET (GtkCellRendererTKClass, accel_cleared),
					 NULL, NULL,
...
/**
 * gtk_cell_renderer_tk_new:
 *
 * Creates a new #GtkCellRendererTK.
 * 
 ...
 */
GtkCellRenderer *
gtk_cell_renderer_tk_new (void)
{
  return g_object_new (GTK_TYPE_CELL_RENDERER_TK, NULL);
}

static gchar *
convert_keysym_state_to_string (GtkCellRendererTK *accel,
				guint                 keysym,
...
static void
gtk_cell_renderer_tk_get_property  (GObject    *object,
                                       guint       param_id,
                                       GValue     *value,
                                       GParamSpec *pspec)
{
  GtkCellRendererTK *accel = GTK_CELL_RENDERER_TK (object);
  ...
  switch (param_id)
    ...
    case PROP_KEYCODE:
      ...
      break;
    case PROP_CELL_MODE:
      g_value_set_enum (value, accel->cell_mode);
      break;
...
static void
gtk_cell_renderer_tk_set_property  (GObject      *object,
                                       guint         param_id,
                                       const GValue *value,
                                       GParamSpec   *pspec)
{
  ...
  switch (param_id)
	...
	if (accel->keycode != keycode)
      ...
      break;
    case PROP_CELL_MODE:
      accel->cell_mode = g_value_get_enum (value);
      break;
...
static void
gtk_cell_renderer_tk_get_size (GtkCellRenderer *cell,
                                  GtkWidget       *widget,
                                  GdkRectangle    *cell_area,
                                  gint            *x_offset,
                                  gint            *y_offset,
...
{
  GtkCellRendererTK *accel = (GtkCellRendererTK *) cell;
  GtkRequisition requisition;

  if (accel->cell_mode == GTK_CELL_RENDERER_TK_CELL_MODE_TEXT) {
	  GTK_CELL_RENDERER_TK_GET_CLASS(accel)->base_get_size(cell, widget, cell_area, x_offset, y_offset, width, height);
	  return;
  }
  ...
  GTK_CELL_RENDERER_CLASS (gtk_cell_renderer_tk_parent_class)->get_size (cell, widget, cell_area,
                   ...
                   GtkCellRendererTK *accel)
{
  ...
  // GDK doesn't consider Super to be a modifier. WTF?
  switch(accel_key) {
	case GDK_Super_L:
	case GDK_Super_R:
	case GDK_Hyper_L:
   ...
   */
  /*
  if (accel->accel_mode == GTK_CELL_RENDERER_TK_MODE_GTK)
...
//#define __GTK_CELL_RENDERER_TK_C__
